...
ALTER TABLE pruzalacusluge
ADD COLUMN IF NOT EXISTS lozinka TEXT DEFAULT '';

CREATE EXTENSION IF NOT EXISTS pgcrypto;

CREATE OR REPLACE FUNCTION func_hash_lozinka()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.lozinka IS NULL OR NEW.lozinka = '' THEN
    NEW.lozinka := crypt(NEW.brlicence, gen_salt('bf', 8));
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER trg_hash_lozinka
BEFORE INSERT ON pruzalacusluge
FOR EACH ROW
EXECUTE FUNCTION func_hash_lozinka();

UPDATE pruzalacusluge
SET lozinka = crypt(brlicence, gen_salt('bf', 8))
WHERE lozinka IS NULL OR lozinka = '';
